...
def get_output(
    ...
    **kwargs: Any,
    ...
    assert result.exit_code == exit_code, build_code_fail_message(
        exit_code, result.exit_code, output, result.exception
    )
    if exception_type:
        assert isinstance(result.exception, exception_type), (
            build_exception_type_message(
                result.exception,
                exception_type,
                output,
            )
    ...
    if exception_pattern:
        assert re.search(exception_pattern, str(result.exception)), (
            build_exception_pattern_message(
                result.exception,
                exception_pattern,
                output,
            )
    ...
    **kwargs: Any,
    ...
    if unwanted_substring:
        ...
        for us in unwanted_substring:
            assert us not in output, build_substring_fail_message(
                us, output, negative_match=True
            )
    ...
    **kwargs: Any,
    ...
    if expected_pattern:
        ...
        for ep in expected_pattern:
            ...
            if escape_parens:
                mangled_pattern = mangled_pattern.replace("(", r"\(").replace(
                    ")", r"\)"
                )
            if escape_brackets:
                mangled_pattern = mangled_pattern.replace("[", r"\[").replace(
            ...
            assert match, build_pattern_fail_message(
                ep, mangled_pattern, output, mangled_output
            )

        if enforce_order:
            assert all(left <= right for (left, right) in pairwise(start_positions)), (
                build_order_fail_message(expected_pattern, start_positions, output)
            )
    ...
    if unwanted_pattern:
        ...
        for up in unwanted_pattern:
            ...
            assert not re.search(mangled_pattern, mangled_output), (
                build_pattern_fail_message(
                    up,
                    mangled_pattern,
                    output,
                    mangled_output,
...
def build_order_fail_message(
    expected_patterns: list[str], start_positions: list[int], output: str
) -> str:
    return conjoin(
